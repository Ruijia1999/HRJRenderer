//
// Created by 黄睿佳 on 2020/12/2.
//

#ifndef HRJRENDERER_GEOMETRY_H
#define HRJRENDERER_GEOMETRY_H
...
template<int n> vec<n> operator^(const vec<n>& lhs, const vec<n>& rhs){
    vec<n> ret = lhs;
    if(n==3){
        ret = vec<n>(lhs[1]*rhs[2]-lhs[2]*rhs[1],lhs[2]*rhs[0]-lhs[0]*rhs[2],lhs[0]*rhs[1]-lhs[1]*rhs[0]);
    }
    return ret;
...
//    mat() = default;
//    vec<4> rows[4]{};
//    mat(double tan, double aspect, double near, double far){
//        rows[0]={1/(aspect*tan),0,0,0,};
//        rows[1]={0,1/tan,0,0};
//        rows[2]={0,0,(-near-far)/(near),1};
//        rows[3]={0,0,(2*near*far)/(near-far),0};
//
//    }
//
//
