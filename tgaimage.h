//
// Created by 黄睿佳 on 2020/11/30.
//

#ifndef HRJRENDERER_TGAIMAGE_H
#define HRJRENDERER_TGAIMAGE_H
...
struct TGAColor {
    ...
    bool operator ==(const TGAColor color) const{
       //std::cout<<(unsigned )color.bgra[0];//<<this->bgra[0]<<color.bgra[1]<<this->bgra[1]<<color.bgra[2]<<this->bgra[2]<<color.bgra[3]<<this->bgra[3]<<std::endl;
        if((unsigned )color.bgra[0]==(unsigned )this->bgra[0]&&(unsigned )color.bgra[1]==(unsigned )this->bgra[1]&&(unsigned )color.bgra[2]==(unsigned )this->bgra[2]&&(unsigned )color.bgra[3]==(unsigned )this->bgra[3]){
            return true;
        }
        return false;
    }
